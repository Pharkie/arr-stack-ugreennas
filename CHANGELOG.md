...
- **Fix Gluetun VPN check command**: Changed `grep -i "connected"` to `grep "Public IP address"` — Gluetun (WireGuard) never logs "connected", it logs the public IP on successful connection
